...
export const weatherDataStore = defineStore({
  ...
  actions: {
    async fetchWeatherData(city) {
      if(city) {
        let response = await fetch(`${api_url}?q=${this.cityInput}&units=${this.unitsType}`)
        this.data = await response.json()
      } else if(this.longitudePos && this.latitudePos){
        let response = await fetch(`${api_url}?lon=${this.longitudePos}&lat=${this.latitudePos}&units=${this.unitsType}`)
        this.data = await response.json()
      }
        ...
        const weatherLocationDate = new Date(this.data.dt * 1000 + (localeTZ + weatherTZ) * 3600000)
        const sunriseDate = new Date(this.data.sys.sunrise * 1000 + (localeTZ + weatherTZ) * 3600000)
        const sunsetDate = new Date(this.data.sys.sunset * 1000 + (localeTZ + weatherTZ) * 3600000)
        this.weatherInfo.cityName = this.data.name
        this.cityInput = this.data.name
        this.weatherInfo.countryName = this.data.sys.country
        this.weatherInfo.weatherCondition = this.capitalizeWords(this.data.weather[0].description)
        ...
        this.weatherInfo.weatherCondition = this.capitalizeWords(`${this.data.message}`)
